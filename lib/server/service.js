  ...
  async invoke(ctx, req, res) {
    ...
    if (!is.asyncFunction(method)) {
      ...
      try {
        ...
        result = err;
        if (!err.ignoreLog) {
          const logger = ctx && ctx.logger || this.logger;
          logger.error(err);
        }
      }
    ...
    if (timeout && res.meta.rt >= timeout) {
      ...
      const logger = ctx && ctx.logger || this.logger;
      logger.warn('[RpcService] service: %s#%s() process timeout, rt: %s, timeout: %s',
    ...
    if (res.isClosed) {
      const logger = ctx && ctx.logger || this.logger;
      logger.warn('[RpcService] client maybe closed before sending response, remote address: %s',
        res.remoteAddress);
      return;
    }

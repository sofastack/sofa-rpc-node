  ...
  async invoke(ctx, req, res) {
    ...
    if (!is.asyncFunction(method)) {
      ...
      try {
        ...
        result = err;
        if (!err.ignoreLog) {
          (ctx && ctx.logger || this.logger).error(err);
        }
      }
    ...
    if (timeout && res.meta.rt >= timeout) {
      // 尽可能使用 ctx logger 打印，确保 tracing 数据正确
      (ctx && ctx.logger || this.logger).warn(
        '[RpcService] service: %s#%s() process timeout, rt: %s, timeout: %s',
        this.id, methodName, res.meta.rt, timeout);
      res.meta.resultCode = '03';
      // 已经超过客户端要求的响应时间，不要再返回数据
      return;
    }
    if (res.isClosed) {
      (ctx && ctx.logger || this.logger).warn(
        '[RpcService] client maybe closed before sending response, remote address: %s',
        res.remoteAddress);
      return;
    }

...
class RpcConsumer extends Base {
  ...
  async invoke(method, args, options = {}) {
    if (!this._isReady) {
      const readyTimeout = options.responseTimeout || this.options.responseTimeout
      await pTimeout(this.ready(), readyTimeout, ()=> {
        // 清空 _readyCallbacks 回调避免内存泄露，但保留将 _isReady 设为 true 的回调函数
        this._readyCallbacks.splice(1, Infinity);
        throw new Error(`[RpcConsumer] Consumer ready timeout for ${readyTimeout}ms`)
      })
    }

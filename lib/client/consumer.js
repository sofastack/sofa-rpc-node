...
const _handleOneway = Symbol.for('RpcConsumer#handleOneway');
...
class RpcConsumer extends Base {
  ...
  get onewayHandler() {
    if (!this[_handleOneway]) {
      this[_handleOneway] = compose(this._middlewares.concat(this._oneway.bind(this)));
    }
    return this[_handleOneway];
  }
  ...
  handleRequest(ctx) {
    const requestHandler = ctx.req.oneway ? this.onewayHandler : this.invokeHandler;

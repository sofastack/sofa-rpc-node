...
const defaultLoadBalancer = require('./loadbalancer');
...
class AddressGroup extends Base {
  constructor(options = {}) {
    ...
    const createLoadBalancer = options.createLoadBalancer || defaultLoadBalancer;
    ...
    this.ready(err => {
      if (!err && !this._closed) {
        ...
        // 保证 同一个 AddressGroup ready 时只监听一次 next 事件。
        if (this._healthCounter.listenerCount('next') === 0) {
  ...
  checkHealthy(address) {
    ...
    const isHealthy = this.connectionManager.doGetConn(address) && !unHealthy;
    if (isHealthy) {
      // healthy
      return 1;
    } else if (unHealthy) {
